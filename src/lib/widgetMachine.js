...
export const widgetMachine = (tokenId, contractAddress, platform) => {
  ...
  return createMachine({
    ...
    states: {
        ...
        invoke: {
          src: () =>
            Promise.all([
              ...
              getArweaveData(tokenId, platform),
            ]),
          onDone: [
            {
              target: "started",
              cond: (context, event) => {
                ...
                if (
                  ...
                  context.display.owner = shortenEthAddr(context.owner.owner_address, platform) || ""
                  context.display.ownerLink = context.baseLink + context.owner.owner_address
      ...
      decrypting: {
        on: { SUCCESS: "display", REJECT: "decrypt_error", DISCONNECT_WALLET: "started" },
      },
      decrypt_error: {},
      signing: {
        on: {
